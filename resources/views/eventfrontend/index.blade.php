...
@section('content')
    <style>
        ...
        .tranding-events-wrapper .geodir-category-img img {
            height: 200px;
            background: no-repeat;
            object-fit: cover;
        }

        .tips-wrapper .card-post-content,
        .tranding-events-wrapper .geodir-category-content {
        ...
        .tranding-events-wrapper .geodir-category-content h3,
        .tranding-events-wrapper .geodir-category-content p {
            width: 310px;
            height: 27px;
            overflow: hidden;
            text-overflow: ellipsis;
                                ...
                                <a href="#">
                                    <div class="geodir-category-img">
                                        <img src="{{ $category->icon }}" alt=""
                                            style="height: 50px;padding: 0px 40%;margin-top: 15px;">
                                        <div class="overlay"></div>
                                    </div>
                                </a>
                        ...
                        <div class="listing-item">
                            <article class="geodir-category-listing fl-wrap">
                                <a href="{{ URL::to('/event/details/' . $trendEvent->id) }}">
                                    <div class="geodir-category-img">
                                        <img src="{{ asset('images/' . $trendEvent->event_image) }}" alt=""
                                            style="height: 200px;">
                                        <div class="overlay"></div>
                                    </div>
                                </a>
                                <div class="geodir-category-content fl-wrap">
                                    <a class="listing-geodir-category"
                                        href="{{ URL::to('/event/details/' . $trendEvent->id) }}">{{ strtoupper($trendEvent->event_type) }}</a>
                                    <h3><a
                                            href="{{ URL::to('/event/details/' . $trendEvent->id) }}">{{ $trendEvent->event_name }}</a>
                                    </h3>
                                    <p>{{ $trendEvent->venue_name }} | {{ $trendEvent->event_location }}</p>
                                    <p><i class="fa fa-location-arrow"></i> <span style="cursor:pointer"
                                            class="get_distance" id="{{ $trendEvent->id }}">
                                            {{ Str::limit(App\Http\Controllers\FrontendController::getDistance($userLat, $userLon, $trendEvent->latitude, $trendEvent->longitude), 4) }}
                                            KM Away</span></p>
                                    <p id="long_{{ $trendEvent->id }}" style="display:none">
                                        {{ $trendEvent->longitude }}</p>
                                    <p id="lat_{{ $trendEvent->id }}" style="display:none">{{ $trendEvent->latitude }}
                                    </p>
                                    <div class="geodir-category-location"><a
                                            href="{{ URL::to('/event/details/' . $trendEvent->id) }}"><i
                                                class="fa fa-map-marker" aria-hidden="true"></i>
                                            {{ $trendEvent->address }}</a></div>
                                    <div class="geodir-category-options fl-wrap">
                                        <p><i class="fa fa-calendar-check-o"></i>
                                            <span>{{ $trendEvent->event_start_date }} |
                                                {{ $trendEvent->event_start_time }}</span></p>
                                    </div>
                                </div>
                            </article>
                        </div>
                    ...
                    <div class="slick-slide-item">
                        ...
                        <div class="listing-item">
                            <article class="geodir-category-listing fl-wrap">
                                ...
                                <div class="geodir-category-content fl-wrap">
                                    <a class="listing-geodir-category"
                                        href="{{ URL::to('/event/details/' . $event->id) }}">{{ strtoupper($event->event_type) }}</a>
                                    <h3><a
                                            href="{{ URL::to('/event/details/' . $event->id) }}">{{ $event->event_name }}</a>
                                    </h3>
                                    <p>{{ $event->venue_name }} | {{ $event->event_location }}</p>
                                    <p><i class="fa fa-calendar-check-o"></i> <span>{{ $event->event_start_date }} |
                                            {{ $event->event_start_time }}</span></p>
                                    <p><i class="fa fa-location-arrow"></i> <span style="cursor:pointer"
                                            class="get_distance" id="{{ $event->id }}">
                                            {{ Str::limit(App\Http\Controllers\FrontendController::getDistance($userLat, $userLon, $event->latitude, $event->longitude), 4) }}
                                            KM Away</span></p>
                                    <p id="long_{{ $event->id }}" style="display:none">{{ $event->longitude }}</p>
                                    <p id="lat_{{ $event->id }}" style="display:none">{{ $event->latitude }}</p>
                                    <div class="geodir-category-options fl-wrap">
                                        <div class="geodir-category-location"><a
                                                href="{{ URL::to('/event/details/' . $event->id) }}"><i
                                                    class="fa fa-map-marker" aria-hidden="true"></i>
                                                {{ $event->address }}</a></div>
                                    </div>
                                </div>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="color-bg-text">
                        ...
                        <a href="https://play.google.com/store/apps/details?id=com.app.peepalike&showAllReviews=true"
                            class="color-bg-link"><i class="fa fa-play"></i> &nbsp;&nbsp;&nbsp;GOOGLE PLAY STORE</a>
                        <a href="https://apps.apple.com/ke/app/peepalike/id1560568386" class="color-bg-link"
                            style="margin-left:20px"><i class="fa fa-apple"></i> &nbsp;&nbsp;&nbsp;APP STORE</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="tips-wrapper">
                            ...
                            <div class="listing-item">
                                <article class="card-post">
                                    <div class="card-post-img fl-wrap">
                                        <a href="/blogs/details/{{ $value->id }}"
                                            onclick="ViewsCount({{ $value->id }})">
                                    ...
                                    <div class="card-post-content fl-wrap">
                                        ...
                                        <div class="blog_extra_detail">
                                            <div class="post-author"><a href="#"><span>By ,
                                                        {{ $value->blog_author }}</span></a></div>
                                            <div class="post-opt">
                                                <ul>
                                                    <li><i class="fa fa-calendar-check-o"></i>
                                                        <span>{{ date('d M Y', strtotime($value->created_at)) }}</span></li>
                                                    <li><i class="fa fa-eye"></i> <span>{{ $value->views }}</span>
                                                    </li>
                                                    <li><i class="fa fa-tags"></i> <a
                                                            href="#">{{ $value->category_type }}</a> </li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
        ...
        <a href="/blogs" class="btn  big-btn circle-btn  dec-btn color-bg flat-btn">Read All<i
                class="fa fa-eye"></i></a>
    </section>
    <!-- section end -->
@endsection
    ...
    <script>
        ...
        $(function() {
            ...
            $(document).on('click', '.get_distance', function() {
                ...
                let total_dist = calcDistance(event_lat, event_long, user_lat, user_long).toFixed(2) +
                    "KM Away";
